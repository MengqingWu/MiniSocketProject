...
void myserver::listen(bool unlimited){
...
void myserver::dataHeadTail (uint32_t size )
{
  ...
  if (size == 0xFFFFFFFF) {
    ...
    if (m_datatype>=0 && m_datatype<6) write(dataFileFd_, &size, 4);  /*xml/raw data*/	  
...
int myserver::poll(){
  ...
  string kpixfile("./kpixNetData.bin");
